...
import {BiErrorCircle, BiHeadphone} from 'react-icons/bi'
...
const NowPlaying = () => {
  ...
  return (
    <div className='nowPlayingCard'>
      ...
      {playerState === 'PLAY' ? <img src='./images/soundbar.gif' title='Now Listening'/> : playerState === 'PAUSE' ? <AiOutlinePauseCircle size={40} /> : playerState === 'OFF' ? <HiOutlineStatusOffline size={40}/> : <BiErrorCircle size={40}/> }</div>
      {/* <div>Song URL: <a href={nowPlaying.songUrl}>{nowPlaying.songUrl}</a></div> */}
    </div>
  );
};
